@javascript
Feature: Prospect customer completes demand check and adds existing contracts
  As a prospect customer
  I want to complete demand check and then adds existing contracts

  Scenario Outline: Prospect customer completes demand check and then adds existing contracts
    ...
    When user clicks on "Verträge ergänzen" link
  ...
  Scenario Outline: user completes the Demandcheck and obtains suitable recommendations for mortgage
    ...
    # Question 4
    When user selects "<house_building>" questionnaire option
    Then user sees "Hast du bereits deine Traum-Immobilie gefunden und suchst nun eine Finanzierungsmöglichkeit?" question label
    ...
    When user clicks on popular option card "Allianz Versicherung"
    Then user is on the clark 2 select category page
    And  user sees contract card "Hausrat - Allianz Versicherung"

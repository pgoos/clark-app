...
require "composites/salesforce/repositories/events/payloads/opportunity_lost_repository"

RSpec.describe Salesforce::Repositories::Events::Payloads::OpportunityLostRepository do
  subject(:repository) { described_class.new }

  let!(:user) { create(:user, last_sign_in_at: DateTime.current) }
  let!(:mandate) { create(:mandate, :accepted, user: user) }
  let!(:product) { create(:product) }
  let!(:opportunity) { create(:opportunity, :lost, mandate: mandate) }
  ...
  describe "#wrap" do
    it "returns opportunity lost event" do
      ...
      expect(event.reason).to eq opportunity.loss_reason
      expect(event.source_description).to eq opportunity.source_description
      expect(event.welcome_call).to eq false
      expect(event).to be_kind_of Salesforce::Entities::Events::OpportunityLost
    end

...
RSpec.describe ClarkAPI::V4::Entities::Offer do
  ...
  context "offer options", :integration do
    ...
    it do
      raw_price = cheapest_option.product.premium_price
      expect(subject).to expose(:cheapest_option_price).of(offer_with_options)
        .as(Hash)
                                                       .with_value(Platform::CurrencyFormatter.format(raw_price))
    end

    it do
      period = cheapest_option.product.premium_period
      expect(subject).to expose(:form_of_payment).of(offer_with_options)

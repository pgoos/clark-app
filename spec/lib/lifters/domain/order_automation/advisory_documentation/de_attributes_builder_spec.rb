...
RSpec.describe Domain::OrderAutomation::AdvisoryDocumentation::DeAttributesBuilder do
  subject do
    described_class.new(
  ...
  describe "#locals" do
    ...
    it "returns proper hash" do
      expect(locals.keys).to eq(
        %i[path_to_footer mandate product admin doc_date introduction company_data
           ...
           advisory_decision documents liability_note answers category_name insurance_situation
           recommendation reason_for_consultation pros_and_cons]
      )
    end
    ...
    it "returns properly formatted locals for generator" do
      ...
      expect(locals[:mediation_info].keys)
        .to eq %i[heading general_insurances life_insurances general_info override_text]
    ...
    context "custom customer_decision provided" do
      subject do
        described_class.new(
          product: product, admin: admin, old_product: old_product,
          opportunity: opportunity, opportunity_source: [], legal_information: { customer_decision: decision }
        )
      end

      let(:decision) { "whatever" }

      it "is populated in advisory decision" do
        expect(locals[:advisory_decision][:decision]).to eq decision
      end
    end

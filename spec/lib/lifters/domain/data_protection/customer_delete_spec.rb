...
RSpec.describe Domain::DataProtection::CustomerDelete do
  ...
  describe "#call" do
    it "sets status to mandate and runs backgroud job" do
      ...
      expect(::DataProtection::CustomerDeleteJob).to receive :perform_later
      described_class.call(admin, mandate)
      expect(mandate.info).to eq("wizard_steps" => [], "status_data_protection" => "CustomerDelete")
    end
  end

  it "returns #name" do
    expect(described_class.strategy_name).to eq "CustomerDelete"
  end

  it "returns #command_job" do
    expect(described_class.command_job).to eq ::DataProtection::CustomerDeleteJob
  end
  ...
  describe ".notice_running_message" do
    it "returns message for running state" do
      expect(described_class.notice_running_message).to(
        eq(I18n.t("admin.mandates.delete.in_progress"))
      )
    end
  end
  ...
  describe ".notice_started_message" do
    it "returns message for started state" do
      expect(described_class.notice_started_message).to(
        eq(I18n.t("admin.mandates.delete.started"))
      )
    end
  end
end

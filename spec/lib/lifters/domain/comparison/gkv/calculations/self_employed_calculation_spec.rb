...
RSpec.describe Domain::Comparison::Gkv::Calculations::SelfEmployedCalculation do
  let(:premium_percentage) { 1.0 }
  ...
  it "gives the right sum based on salary and zuzatsbeitrag" do
    calculation = described_class.new(salary)
    expect(calculation.calculate(premium_percentage)).to eq(Money.new("520", "EUR"))
  end
end

...
describe RoboAdvisor, :integration do
  ...
  context "BU + Pension combo" do
    let(:category) do
      create(:category, ident: described_class::BU_PENSION_MULTI_CATEGORY_IDENTS.sample)
    end
    ...
    it_behaves_like "a robo advice for method", :disability_with_pension_combo

    it "advices all products in the list" do
      expect do
        subject.disability_with_pension_combo
      end.to change { product.interactions.count }

      i18n_key = "robo_advisor.disability_with_pension_combo.catch_all"

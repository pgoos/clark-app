...
describe RoboAdvisor, :integration do
  ...
  context "Motorbike Insurance" do
    let(:category) do
      create(:category, ident: described_class::MOTORBIKE_INSURANCE)
    end
    ...
    it_behaves_like "a robo advice for method", :motorbike_insurance

    it "advices all products in the list" do
      expect do
        subject.motorbike_insurance
      end.to change { product.interactions.count }

      i18n_key = "robo_advisor.motorbike_insurance.catch_all"

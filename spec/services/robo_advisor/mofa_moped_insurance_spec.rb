...
describe RoboAdvisor, :integration do
  ...
  context "Mofa Moped Insurance" do
    let(:category) { create(:category, ident: RoboAdvisor::MOFA_MOPED_IDENT) }
    ...
    it_behaves_like "a robo advice for method", :mofa_moped_insurance

    it "advices all products" do
      expect do
        subject.mofa_moped_insurance
      end.to change { product.interactions.count }

      i18n_key = "robo_advisor.mofa_moped_insurance.catch_all"

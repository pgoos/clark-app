...
describe RoboAdvisor, :integration do
  ...
  context "Haus- & Grundbesitzerhaftpflichtversicherung" do
    let(:category) do
      create(:category, ident: described_class::HOUSE_LAND_OWNER_PHV_INSURANCE.sample)
    end
    ...
    it_behaves_like "a robo advice for method", :house_landowner_private_liability_insurance

    it "advices all products in the list" do
      expect do
        subject.house_landowner_private_liability_insurance
      end.to change { product.interactions.count }

      i18n_key = "robo_advisor.house_landowner_private_liability_insurance.catch_all"

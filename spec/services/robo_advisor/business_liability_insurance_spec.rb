...
describe RoboAdvisor, :integration do
  ...
  context "Motorbike Insurance" do
    let(:category) do
      create(:category, ident: described_class::BUSINESS_LIABILITY_INSURANCE)
    end
    ...
    it_behaves_like "a robo advice for method", :business_liability_insurance

    it "advices all products in the list" do
      expect do
        subject.business_liability_insurance
      end.to change { product.interactions.count }

      i18n_key = "robo_advisor.business_liability_insurance.catch_all"

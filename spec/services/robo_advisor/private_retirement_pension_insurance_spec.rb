...
describe RoboAdvisor, :integration do
  ...
  context "Private Renten Insurance" do
    let(:category) do
      create(:category, ident: described_class::PRIVATE_RETIREMENT_CATEGORY)
    end
    ...
    it_behaves_like "a robo advice for method", :private_retirement_pension_insurance

    it "advices all products in the list" do
      expect do
        subject.private_retirement_pension_insurance
      end.to change { product.interactions.count }

      i18n_key = "robo_advisor.private_retirement_pension_insurance.catch_all"

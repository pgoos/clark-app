...
RSpec.describe ArisecurTransferJob, :integration, type: :job do
  let(:product) { create(:product) }
  let(:result) do
    instance_double(Utils::Interactor::Result)
  end
  let(:transfer_service_double) do
    instance_double(Carrier::Constituents::Arisecur::Interactors::StockTransfer, call: result)
  end
  let(:worker) { Delayed::Worker.new }

  before do
    allow(Carrier::Constituents::Arisecur::Interactors::StockTransfer)
      .to receive(:new).and_return(transfer_service_double)
    allow(transfer_service_double).to receive(:call)
  end
  ...
  context "when executing the job" do
    it "calls the correct service" do
      ...
      expect(transfer_service_double).to have_received(:call).with(product.id)
    end
  end
  ...
  context "with a limit on the retry" do
    it "does not exceed the max attempts" do
      allow(transfer_service_double).to receive(:call).and_raise("Error")
      described_class.perform_later(product.id)
  ...
  context "when updating metadata" do
    let(:errors) { ["error1"] }
    let(:transfer_error) do
      Carrier::Constituents::Arisecur::Interactors::Errors::StockTransferError.new(errors: errors)
    end
    ...
    it "raises an error with an error in the service" do
      allow(transfer_service_double).to receive(:call).and_raise(transfer_error)
  ...
  context "when there is more than one waiting job" do
    let(:product) { create(:product) }
    let!(:jobs) { create_list(:delayed_job, 2, delayed_reference: product, queue: "arisecur_transfer") }
  ...
  context "when reenqueuing a job" do
    let(:product) { create(:product) }
    let!(:job) { create(:delayed_job, delayed_reference: product, queue: "arisecur_transfer") }
